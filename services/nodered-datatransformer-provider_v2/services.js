...
export function installNode(config,datatransformerId){
    let moduleName = config.module;
    let repositoryUrl = config.repositoryUrl;
    let origin = config.origin; //npm or private
    let nodeUrl = "";

    console.log("Installing node to "+datatransformerId+"...");

    if(origin == "npm"){
        return getDataTransformers(datatransformerId).then((datatransformers)=>{
            nodeUrl = datatransformers[0].url;
            ...
            return rp(options)
                ...
                .catch(function (err) {
                    ...
                    if(err.message.includes("Module already loaded")) return {code: 400, message: "Module already loaded"};
                    else return {code: 400, message: err.message};
                });	
        });
        ...
        return getDataTransformers(datatransformerId).then((datatransformers)=>{
            nodeUrl = datatransformers[0].url;
            return exec(`docker ps -a | grep ${datatransformerId} | head -1 | cut -c 1-12`);
            

        }).then((res)=>{
            let dockerID = res.stdout.trim();
            return exec(`docker exec ${dockerID} git clone ${repositoryUrl} /usr/src/node-red/${moduleName}`);
        }).then((res)=>{

            var _body = {
                "module": "/usr/src/node-red/"+moduleName
            };
            var options = {
                method: 'POST',
                uri: nodeUrl+"/nodes",
            ...
            if(err.stderr.includes("already exists and is not an empty directory")) return {code: 400, message: "Module already loaded"};
